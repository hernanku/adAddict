...
module.exports = (app, config, ad) => {
  ...
  app.put('/user/:user/extend', async (req, res) => {
    req.body = parseBody(req.body);
    const user = req.params.user;
    let [error, data] = await wrapAsync(ad.setUserProperty(user,req.body));
    let response = !error ? { success: true } : undefined;
    respond(res, error, response);
  });

  app.put('/user/:user/setproperty', async (req, res) => {
    req.body = parseBody(req.body);
	var arrError = [];
    const user = req.params.user;
	for(const key of Object.keys(req.body)) {
                let obj = { [key]: req.body[key] }
		let [error, data] = await wrapAsync(ad.setUserProperty(user,obj));
		let err = !error ? { success: true } : null;
		if(err == null){
			arrError.push(err);
		}
	}
	let response = arrError.len == 0 ? { success: true } : undefined;
    //let response = !error ? { success: true } : undefined;
    respond(res, arrError, response);
  });
